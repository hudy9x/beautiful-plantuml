...
/**
 * Shows when a group block is selected.
 * Replace with your own component to customize the group context menu.
 */
export function GroupMenuBar() {
  ...
  if (!node || node.type !== "GROUP_BLOCK") return null;
    ...
    <MenuPopover position={clickPosition} title="Group Actions" subtitle={<span>{node.label || "(no label)"}</span>}>
      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", justifyContent: "center" }}>
        <ActionButton icon={<EditIcon width={14} height={14} />} onClick={async () => {
          const newLbl = await actions.prompt("Edit group label:", node.label);
